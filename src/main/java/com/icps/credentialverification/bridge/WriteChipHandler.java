...
public class WriteChipHandler implements HttpHandler {

    private final ChipReader chipReader;

    public WriteChipHandler(ChipReader chipReader) {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            String payloadB64 = extractJsonValue(body, "payload");

            if (chipUid == null || payloadB64 == null) {
                send(exchange, 400, "{\"error\":\"chip_uid and payload are required.\"}");
                return;
            }

            byte[] payloadBytes = Base64.getDecoder().decode(payloadB64);
            chipReader.writePayload(chipUid, payloadBytes);
